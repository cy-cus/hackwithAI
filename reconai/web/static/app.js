...
function showPage(pageId) {
    ...
    // Highlight nav item if triggered by click
    ...
    grid.innerHTML = TOOLS.map(tool => {
        ...
        <div class="tool-card ${state.selectedTools.has(tool.id) ? 'selected' : ''}" 
            ...
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                ...
                ${state.selectedTools.has(tool.id) ? '<span style="color: var(--accent-green); font-size: 1.25rem;">✓</span>' : ''}
            </div>
...
async function installTool(toolId) {
    showToast(`⏳ Installing ${toolId}...`, 'info');
    try {
...
function toggleTool(toolId) {
    ...
    if (state.selectedTools.has(toolId)) {
        ...
        showToast(`❌ Deselected: ${tool.name}`, 'info');
    } else {
...
function selectAllTools() {
    ...
    loadToolGrid();
    showToast('✅ All tools selected', 'success');
}

function deselectAllTools() {
    state.selectedTools.clear();
    loadToolGrid();
    showToast('❌ All tools deselected', 'info');
}

function saveToolConfig() {
    localStorage.setItem('selectedTools', JSON.stringify([...state.selectedTools]));
    showToast(`✅ Saved ${state.selectedTools.size} tools!`, 'success');
}
...
async function startScan(event) {
    ...
    if (isSubmittingScan) {
        showNotification('⏳ Scan already starting...', 'info');
        return;
    }
    ...
    // Capture limits (allow 0 for unlimited)
    const getLimit = (id, def) => {
        const val = parseInt(document.getElementById(id).value);
        return isNaN(val) ? def : val;
    };

    const limitVuln = getLimit('limit-vuln', 500);
    const limitJs = getLimit('limit-js', 2000);
    const limitFuzz = getLimit('limit-fuzz', 200);
    ...
    const scanConfig = {
        ...
        // Send Custom Limits
        limit_vuln: limitVuln,
        limit_js: limitJs,
        limit_fuzz: limitFuzz,
        ai_config: {
    ...
    try {
        ...
        showNotification(`🚀 Scan "${data.scan_id}" started!`, 'success');

        // Switch to Scans page and START POLLING IMMEDIATELY
        showPage('scans');

        // Clear any existing polling
        if (activeScanPoller) clearTimeout(activeScanPoller);

        // Start polling immediately (don't wait 500ms)
        await loadActiveScans();

        // Also start polling for partial results
        startPartialResultsPolling(data.scan_id);
...
async function loadScanResults(scanId) {
    try {
        const response = await fetch(`${API_BASE}/api/scan/${scanId}/result`); // Fixed endpoint (singular)
        const data = await response.json();
        const results = data.result || {}; // Access nested result object

        // Load all data types
        state.allBugs = results.findings || results.bugs || [];
        state.allSubdomains = results.subdomains || [];
        // Extract URL strings from Endpoint objects (wayback URLs are stored as Endpoints)
        const rawWaybackUrls = results.urls || results.wayback_urls || [];
        state.allWaybackUrls = rawWaybackUrls.map(item => {
            if (typeof item === 'string') return item;
            if (item && item.url) return item.url;
            return String(item);
        });
        state.allJsFiles = results.js_files || [];

        // Update dashboard
        document.getElementById('dash-total-bugs').textContent = results.total_bugs || 0;
        document.getElementById('dash-critical').textContent = results.critical_bugs || 0;
        ...
        // Refresh lists
        renderBugs(state.allBugs);
        renderSubdomains(state.allSubdomains);
        renderWayback(state.allWaybackUrls);
        renderJS(state.allJsFiles);

        showPage('dashboard'); // Or stay on current page? Usually dashboard is summary.
...
function exportSingle() {
    ...
    showNotification('✅ Finding exported!', 'success');
    document.getElementById('context-menu').style.display = 'none';
}
...
// Limit toggle helper
function toggleLimit(checkbox, inputId) {
    ...
    if (checkbox.checked) {
        ...
        input.value = input.getAttribute('data-prev') || (inputId.includes('js') ? "2000" : "500");
...
async function updateDashboard() {
    // 1. Update stats (simplified for now)
    const completedScansObj = JSON.parse(localStorage.getItem('completedScans') || '[]');
    document.getElementById('dash-scans').textContent = completedScansObj.length || 0;

    // 2. Update Active Scan Card
    try {
        const res = await fetch(`${API_BASE}/api/scans/active`);
        const data = await res.json();
        const scans = data.scans || [];

        // Filter for current project active scans
        const currentProject = (state.currentProject || 'Default').toLowerCase();
        const activeScan = scans.find(s =>
            (s.project || 'Default').toLowerCase() === currentProject &&
            ['running', 'paused'].includes(s.status)
        );

        const container = document.getElementById('active-scan-info');
        const statusBadge = document.getElementById('scan-status');

        if (activeScan) {
            // Update Status Badge
            if (statusBadge) {
                statusBadge.textContent = activeScan.status.toUpperCase();
                statusBadge.className = `severity-badge ${activeScan.status === 'running' ? 'severity-critical' : 'severity-medium'}`;
                statusBadge.style.background = activeScan.status === 'running' ? 'rgba(0,255,65,0.2)' : 'rgba(255,165,0,0.2)';
                statusBadge.style.color = activeScan.status === 'running' ? '#00ff41' : 'orange';
            }

            // Render Active Scan Card
            const tool = activeScan.current_tool || 'Initializing';
            const progress = activeScan.progress || 0;
            const msg = activeScan.message || 'Processing...';

            container.innerHTML = `
                <div style="text-align: center;">
                    <div style="margin-bottom:1rem">
                        <div style="font-size:1.5rem;font-weight:700;color:#00ff41;margin-bottom:0.5rem">
                            🎯 ${activeScan.target || activeScan.id}
                        </div>
                        <div style="display:inline-block;padding:0.5rem 1rem;background:rgba(0,255,65,0.1);border:1px solid #00ff41;border-radius:6px;font-weight:600;color:#00ff41;animation:pulse 2s infinite">
                            🔧 ${tool}
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:1rem">
                        <div style="display:flex;justify-content:space-between;margin-bottom:0.25rem;font-size:0.9rem">
                            ...
                            <span style="color:#00ff41;font-weight:bold">${progress}%</span>
                        </div>
                        <div style="height:12px;background:#000;border-radius:6px;overflow:hidden;border:1px solid #00ff41">
                            <div style="height:100%;width:${progress}%;background:linear-gradient(90deg,#00ff41,#00cc33);transition:width 0.5s"></div>
                        </div>
                    </div>
                    
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;font-size:0.85rem">
                        <div style="background:rgba(0,255,200,0.1);padding:0.5rem;border-radius:4px;color:#00ffc8">
                            <b>${activeScan.stats?.subdomains_found || 0}</b> Subdomains
                        </div>
                        <div style="background:rgba(255,165,0,0.1);padding:0.5rem;border-radius:4px;color:orange">
                            <b>${activeScan.stats?.endpoints_found || 0}</b> Endpoints
                        </div>
                    </div>
                    
                    <button onclick="showPage('history')" style="margin-top:1rem;width:100%;padding:0.75rem;background:#00ff41;color:#000;border:none;border-radius:6px;font-weight:700;cursor:pointer">
                        View Full Details
                    </button>
                </div>
            `;
            ...
            if (statusBadge) {
                ...
                statusBadge.className = 'severity-badge severity-medium';
...
function renderSubdomains(subdomains) {
    ...
    populateSubdomainSources();
    filterSubdomains();
}

function populateSubdomainSources() {
    const select = document.getElementById('filter-source');
    if (!select) return;

    // Get unique sources
    const sources = new Set(['ultimate']);
    state.allSubdomains.forEach(sub => {
        const s = sub.source || sub.sources?.join(', ') || 'unknown';
        sources.add(s);
    });
    ...
    // Rebuild options (keep All and Aggregated)
    select.innerHTML = '<option value="all">All Sources</option><option value="ultimate">Aggregated</option>';

    sources.forEach(s => {
        if (s !== 'ultimate' && s !== 'unknown') {
...
function filterSubdomains() {
    ...
    const searchQuery = document.getElementById('subdomain-search')?.value.toLowerCase() || '';
    const sourceFilter = document.getElementById('filter-source')?.value || 'all';
    const liveOnly = document.getElementById('filter-live-only')?.checked || false;

    let filtered = state.allSubdomains.filter(sub => {
        const host = (sub.host || sub.subdomain || sub || '').toLowerCase();
        const source = (sub.source || sub.sources?.join(', ') || 'unknown');
        const resolved = sub.resolved || false;

        // Search
        if (searchQuery && !host.includes(searchQuery)) return false;

        // Live Only
        if (liveOnly && !resolved) return false;
    ...
    displaySubdomainsList(filtered, container);
}

function displaySubdomainsList(subdomains, container) {
    if (subdomains.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-icon">🌐</div><p>No matching subdomains found</p></div>';
        return;
    }

    container.innerHTML = subdomains.map((sub, idx) => {
        const host = sub.host || sub.subdomain || sub;
        let source = sub.source;
        if ((!source || source === 'ultimate' || source === 'ultimate_enum') && sub.sources && sub.sources.length > 0) {
            source = sub.sources.join(', ');
        }
        source = source || 'unknown';

        // Final cleanup
        if (source === 'ultimate' || source === 'ultimate_enum') source = 'Aggregated';
        ...
        <div class="result-item">
            <div class="result-header">
                <div>
                    <div class="result-title">${host}</div>
                    <div class="result-meta">Source: ${source}</div>
                    ${sub.ip ? `<div class="result-meta" style="color:#00ff41">IP: ${sub.ip}</div>` : ''}
                </div>
                ${resolved ? '<span class="severity-badge severity-high">✓ LIVE</span>' : '<span class="severity-badge severity-low">UNVERIFIED</span>'}
            </div>
        </div>
    `;
    }).join('');
...
function exportSubdomains(format) {
    ...
    showNotification('✅ Subdomains exported!', 'success');
}
...
function renderWayback(urls) {
    ...
    if (state.allWaybackUrls.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-icon">📜</div><p>No wayback URLs found</p></div>';
        return;
    }

    container.innerHTML = state.allWaybackUrls.map((urlItem, idx) => {
        // Handle both string URLs and Endpoint objects
        const url = (typeof urlItem === 'string') ? urlItem : (urlItem?.url || String(urlItem));
        return `
        <div class="result-item">
...
function searchWayback(query) {
    const filtered = state.allWaybackUrls.filter(urlItem => {
        const url = (typeof urlItem === 'string') ? urlItem : (urlItem?.url || String(urlItem));
    ...
    const container = document.getElementById('wayback-list');
    if (filtered.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-icon">🔍</div><p>No URLs match your search</p></div>';
        return;
    }

    container.innerHTML = filtered.map(urlItem => {
        // Handle both string URLs and Endpoint objects
        const url = (typeof urlItem === 'string') ? urlItem : (urlItem?.url || String(urlItem));
        return `
        <div class="result-item">
...
function exportWayback(format) {
    ...
    a.click();
    showNotification('✅ Wayback URLs exported!', 'success');
}
...
function renderJS(files) {
    ...
    if (state.allJsFiles.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-icon">💻</div><p>No JS files found</p></div>';
        return;
    }
            ...
            <div class="result-header" style="cursor: pointer; background: ${hasFindings ? 'rgba(0,255,200,0.05)' : 'transparent'}; padding: 1rem;" onclick="document.getElementById('js-details-${idx}').style.display = document.getElementById('js-details-${idx}').style.display === 'none' ? 'block' : 'none'">
                <div style="flex: 1;">
                    <div class="result-title" style="font-size: 1rem; margin-bottom: 0.5rem;">
                        📄 <span style="color: var(--accent-cyan); font-weight: 700;">${js.url || js.path || js}</span>
                    </div>
                    <div class="result-meta" style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: center;">
                        ${js.size ? `<span style="color: var(--text-secondary);">📦 ${(js.size / 1024).toFixed(2)} KB</span>` : ''}
                        ${secrets.length > 0 ? `<span style="padding: 0.25rem 0.75rem; background: rgba(255,68,68,0.2); border: 2px solid #ff4444; border-radius: 4px; color: #ff4444; font-weight: 700;">🔑 ${secrets.length} SECRET${secrets.length > 1 ? 'S' : ''}</span>` : ''}
                        ${endpoints.length > 0 ? `<span style="padding: 0.25rem 0.75rem; background: rgba(0,255,200,0.2); border: 2px solid var(--accent-cyan); border-radius: 4px; color: var(--accent-cyan); font-weight: 700;">🔗 ${endpoints.length} ENDPOINT${endpoints.length > 1 ? 'S' : ''}</span>` : ''}
                        ${dependencies.length > 0 ? `<span style="padding: 0.25rem 0.75rem; background: rgba(255,165,0,0.2); border: 2px solid orange; border-radius: 4px; color: orange; font-weight: 700;">📦 ${dependencies.length} PACKAGE${dependencies.length > 1 ? 'S' : ''}</span>` : ''}
                    </div>
                </div>
                ${hasFindings ? '<span style="font-size: 1rem; color: var(--accent-cyan); font-weight: 700;">▼ EXPAND</span>' : ''}
            </div>
            ...
            <div id="js-details-${idx}" style="display: none; padding: 1.5rem; background: rgba(0,0,0,0.3); border-top: 1px solid var(--border);">
                ...
                <div style="margin-bottom: 2rem;">
                    <h3 style="color: #ff4444; margin-bottom: 1rem; font-size: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
                        🔑 <span>SECRETS FOUND IN THIS FILE</span>
                    </h3>
                ...
                <div style="margin-bottom: 2rem;">
                    <h3 style="color: var(--accent-cyan); margin-bottom: 1rem; font-size: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
                        🔗 <span>API ENDPOINTS FROM THIS FILE</span>
                    </h3>
                ...
                <div>
                    <h3 style="color: orange; margin-bottom: 1rem; font-size: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
                        📦 <span>DEPENDENCIES (Dependency Confusion Risk)</span>
                    </h3>
                    <div style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.75rem;">
                        Source: <span style="color: var(--accent-cyan); font-weight: 600;">${js.url || js.path || 'Unknown'}</span>
                    </div>
...
function searchJS(query) {
    ...
    const container = document.getElementById('js-list');
    if (filtered.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-icon">🔍</div><p>No JS files match your search</p></div>';
        return;
    }
...
function switchProject(projectName) {
    ...
    showToast(`📂 Switched to project: ${projectName}`, 'success');
    updateDeleteButton();
    loadScanHistory();
...
async function checkActiveScans() {
    try {
        ...
        if (myScan) {
            ...
            showToast('🔄 Active scan running! Check "Scans" tab for progress.', 'info');
...
async function deleteCurrentProject() {
    ...
    try {
        ...
        showToast('❌ Deletion failed (Network/JS Error)', 'error');
    }
}
...
async function createProject() {
    ...
    try {
        ...
        console.error(e);
        showToast('❌ Creation failed (JS Error)', 'error');
    }
}


// Partial Results Polling
let partialResultsPoller = null;

function startPartialResultsPolling(scanId) {
    // Clear any existing poller
    if (partialResultsPoller) clearInterval(partialResultsPoller);

    // Poll every 3 seconds
    partialResultsPoller = setInterval(() => loadPartialResults(scanId), 3000);

    // Load immediately too
    loadPartialResults(scanId);
}

async function loadPartialResults(scanId) {
    try {
        const res = await fetch(`${API_BASE}/api/scan/${scanId}/partial-results`);
        const data = await res.json();

        if (data.error) {
            console.log("Partial results not available yet");
            return;
        }
        ...
        if (data.js_files && data.js_files.length > 0) {
            state.allJS = data.js_files;
            renderJS(data.js_files);
        }
        ...
        if (data.endpoints && data.endpoints.length > 0) {
            // Could render endpoints too if needed
        }
    } catch (e) {
        console.error("Failed to load partial results:", e);
    }
}

function stopPartialResultsPolling() {
    if (partialResultsPoller) {
        clearInterval(partialResultsPoller);
        partialResultsPoller = null;
    }
}
                    ...
                    <div style="padding: 1.5rem; background: rgba(0,0,0,0.4); border-bottom: 2px solid ${statusColor};">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                ${status === 'paused'
                        ? `<button class="btn btn-sm" onclick="resumeScan('${scanId}')" style="background: rgba(0,255,100,0.3); color: #00ff64; border: 2px solid #00ff64; font-weight: 700;">▶ RESUME</button>`
                        : `<button class="btn btn-sm" onclick="pauseScan('${scanId}')" style="background: rgba(255,165,0,0.3); color: orange; border: 2px solid orange; font-weight: 700;">⏸ PAUSE</button>`
                    }
                                <button class="btn btn-sm btn-primary" onclick="viewScan('${scanId}')" style="font-weight: 700;">👁 VIEW</button>
                            </div>
                        </div>
                        ...
                        <div style="margin-top: 1rem; display: flex; align-items: center; gap: 1rem;">
                            <span style="padding: 0.5rem 1rem; background: rgba(${statusRgba},0.2); border: 2px solid ${statusColor}; border-radius: 6px; color: ${statusColor}; font-weight: 700; text-transform: uppercase;">
                                ${status === 'paused' ? '⏸ PAUSED' : status === 'running' ? '▶ RUNNING' : status}
                            </span>
                            ${currentTool ? `
...
// ========== EMERGENCY FIX - OVERRIDE BROKEN FUNCTIONS ==========
console.log("=== LOADING EMERGENCY FIX ===");

async function refreshScans() {
    console.log("=== refreshScans called ===");
    await loadActiveScans();
    await loadCompletedScans();
}
...
async function loadActiveScans() {
    try {
        const res = await fetch(`${API_BASE}/api/scans/active`);
        const data = await res.json();
        console.log(`Active scans API returned:`, data);
        ...
        const currentProject = (state.currentProject || 'Default').toLowerCase();
        const projectScans = scans.filter(s => {
            const scanProject = (s.project || 'Default').toLowerCase();
            return scanProject === currentProject;
        });

        console.log(`Showing ${projectScans.length} active scans for project "${state.currentProject}"`);

        if (projectScans.length === 0) {
            container.innerHTML = '<div style="padding:2rem;text-align:center;color:#999">No active scans</div>';
            return;
        }

        container.innerHTML = projectScans.map(scan => {
            const stats = scan.stats || {};
            const tool = scan.current_tool || 'Initializing';
            const progress = scan.progress || 0;
            const msg = scan.message || 'Starting...';

            return `
                <div style="background:linear-gradient(135deg,#1a1a2e,#0f0f1e);border:3px solid #00ff41;border-radius:15px;padding:2rem;margin-bottom:2rem;box-shadow:0 10px 30px rgba(0,255,65,0.3)">
                    <!-- Header: Target & Tool -->
                    <div style="margin-bottom:1.5rem">
                        <div style="font-size:2rem;font-weight:900;color:#00ff41;margin-bottom:0.5rem">
                            🎯 ${scan.target || scan.id}
                        </div>
                        <div style="display:inline-block;padding:0.75rem 1.5rem;background:rgba(0,255,65,0.2);border:2px solid #00ff41;border-radius:8px;font-size:1.3rem;font-weight:700;color:#00ff41;animation:pulse 2s infinite">
                            🔧 ${tool}
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:1.5rem">
                        <div style="display:flex;justify-content:space-between;margin-bottom:0.5rem">
                            <span style="color:#aaa">${msg}</span>
                            <span style="font-size:1.5rem;font-weight:900;color:#00ff41">${progress}%</span>
                        </div>
                        <div style="height:20px;background:#000;border-radius:10px;overflow:hidden;border:2px solid #00ff41">
                            <div style="height:100%;width:${progress}%;background:linear-gradient(90deg,#00ff41,#00cc33);transition:width 0.5s"></div>
                        </div>
                    </div>
                    
                    <!-- Live Stats Grid -->
                    ${Object.keys(stats).some(k => stats[k] > 0) ? `
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem;margin-top:1.5rem">
                        ...
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #00d4ff;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#00d4ff">${stats.subdomains_found}</div>
                            <div style="color:#888;margin-top:0.5rem">Subdomains</div>
                        </div>
                        ` : ''}
                        
                        ${stats.live_hosts > 0 ? `
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #00ff41;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#00ff41">${stats.live_hosts}</div>
                            <div style="color:#888;margin-top:0.5rem">Live Hosts</div>
                        </div>
                        ` : ''}
                        
                        ${stats.endpoints_found > 0 ? `
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #ffa500;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#ffa500">${stats.endpoints_found}</div>
                            <div style="color:#888;margin-top:0.5rem">Endpoints</div>
                        </div>
                        ` : ''}
                        
                        ${stats.js_files_found > 0 ? `
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #ffd700;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#ffd700">${stats.js_files_found}</div>
                            <div style="color:#888;margin-top:0.5rem">JS Files</div>
                        </div>
                        ` : ''}
                        
                        ${stats.secrets_found > 0 ? `
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #ff4444;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#ff4444">${stats.secrets_found}</div>
                            <div style="color:#888;margin-top:0.5rem">Secrets</div>
                        </div>
                        ` : ''}
                        
                        ${stats.bugs_found > 0 ? `
                        <div style="background:#0a0a1a;padding:1.5rem;border-radius:10px;border-left:5px solid #ff006e;text-align:center">
                            <div style="font-size:3rem;font-weight:900;color:#ff006e">${stats.bugs_found}</div>
                            <div style="color:#888;margin-top:0.5rem">Bugs</div>
                        </div>
                        ` : ''}
                    </div>
                    ` : ''}
        ...
        setTimeout(loadActiveScans, 2000);

    } catch (e) {
        console.error("ERROR loading active scans:", e);
    }
}

async function loadCompletedScans() {
    console.log("loadCompletedScans: START");
    try {
        const url = `${API_BASE}/api/scans`;
        console.log("Fetching completed scans:", url);
        ...
        const container = document.getElementById('completed-scans-list');
        if (!container) {
            console.error("Container 'completed-scans-list' NOT FOUND");
            return;
        }

        const scans = data.scans || [];

        // STRICT PROJECT FILTERING (Case-insensitive)
        const currentProject = (state.currentProject || 'Default').toLowerCase();

        const projectScans = scans.filter(s => {
            const scanProject = (s.project || 'Default').toLowerCase();
            const isCompleted = s.status !== 'running' && s.status !== 'paused';
            return scanProject === currentProject && isCompleted;
        });

        console.log(`Filtering scans for project '${state.currentProject}': Found ${projectScans.length} of ${scans.length} total`);

        if (projectScans.length === 0) {
            // Check if we have scans for other projects to show a helpful message
            const otherScansCount = scans.filter(s => s.status !== 'running' && s.status !== 'paused').length;

            container.innerHTML = `
                <div style="padding:3rem;text-align:center;color:#666">
                    <div style="font-size:3rem;margin-bottom:1rem">📂</div>
                    <div style="font-size:1.2rem;margin-bottom:0.5rem">No completed scans for project <b>${state.currentProject}</b></div>
                    ${otherScansCount > 0 ? `<div style="font-size:0.9rem;color:#999">(${otherScansCount} scans in other projects)</div>` : ''}
                </div>`;
            return;
        }

        container.innerHTML = projectScans.map(scan => `
            <div onclick="viewScan('${scan.id}')" style="padding:1.5rem;margin-bottom:1rem;background:rgba(0,0,0,0.3);border-left:5px solid ${scan.status === 'completed' ? '#00ff64' : '#ff4444'};border-radius:6px;cursor:pointer;transition:all 0.3s" onmouseover="this.style.background='rgba(0,255,100,0.1)'" onmouseout="this.style.background='rgba(0,0,0,0.3)'">
                <div style="font-size:1.3rem;font-weight:700;color:#00ff64;margin-bottom:0.5rem">${scan.target || scan.id}</div>
                <div style="font-size:0.9rem;color:#999">${scan.status} • ${scan.completed_at ? new Date(scan.completed_at).toLocaleString() : 'Unknown'}</div>
                <div style="margin-top:0.5rem;font-size:0.8rem;color:#666">Project: ${scan.project || 'Default'}</div>
            </div>
        `).join('');

        console.log("✓ Completed scans rendered");
    } catch (e) {
        console.error("ERROR in loadCompletedScans:", e);
    }
}
